  ...
  return function stoch_osc (s, key, length) {
    ...
    if (s.lookback.length >= length) {
      ...
      if (s.options.auto_stoch_osc && s.lookback[0].stoch_osc) {
        get('lib.stddev')(s, 'stoch_osc_stddev', Math.floor(s.options.stoch_osc_periods * 3), 'stoch_osc')
        s.options.overbought_stoch_osc = 50 + (.8 * s.period.stoch_osc_stddev)
        s.options.oversold_stoch_osc = 50 - (.8 * s.period.stoch_osc_stddev)
      }
    }
  }
}
