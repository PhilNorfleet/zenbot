...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop,trade) {
      var stop_signal

      if (!s.acted_on_stop && s.lookback[0]){
        var dif = ((trade.price - s.lookback[0].close) / s.lookback[0].close)
        // console.log('\ntrade price-' + trade.price + ' lookback-' + s.lookback[0].close + ' dif- ' + dif*100)

          if (so.buy_drop_pct && (dif < 0) && (so.buy_drop_pct < Math.abs(dif) * 100)) {
            stop_signal = 'buy'
            console.log(('\nbuy drop triggered at -' + dif*100 + '%' + ' price change\n').red)
          }
          if (so.sell_pop_pct && (dif > 0) && (so.sell_pop_pct < dif * 100)) {
            stop_signal = 'sell'
            console.log(('\nsell pop triggered at +' + dif*100 + '%' + ' price change\n').green)
          }
      ...
      if (s.my_trades.length) {
        ...
        if (!s.acted_on_stop) {
          ...
          else if (s.buy_stop && s.period.close > s.buy_stop) {
              stop_signal = 'buy'
    ...
    function writeReport (is_progress, blink_off) {
      ...
      get('indicators.rsi')(s, 'rsi', so.rsi_periods)
