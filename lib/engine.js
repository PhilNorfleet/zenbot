  ...
  return function (s) {
    ...
    function executeStop (do_sell_stop) {
      ...
      if (s.lookback[0]){
        ...
        if (!s.acted_on_stop) {
          if (so.buy_drop_pct && (diff < 0) && (so.buy_drop_pct < Math.abs(diff) * 100)) {
            stop_signal = 'buy'
            console.log(('\nbuy drop triggered at -' + diff + ' price change\n').red)
          }
          if (so.sell_pop_pct && (diff > 0) && (so.sell_pop_pct < diff * 100)) {
            stop_signal = 'sell'
            console.log(('\nsell pop triggered at +' + diff + ' price change\n').green)
          }
        }
      }
      if (!s.acted_on_stop && so.buy_drop_pct && (diff < 0) && (so.buy_drop_pct < Math.abs(diff) * 100)) {
              stop_signal = 'buy'
              console.log(('\nbuy drop triggered at -' + diff + ' price change\n').red)
          }
